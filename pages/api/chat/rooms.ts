  ...
  try {
    if (req.method === "GET") {
      ...
      if (req.session.role !== "ADMIN") {
        return res.status(200).json({ rooms: memberRooms });
      }

      const memberRoomIds = memberRooms.map((room) => room.id);
      const adminRooms = await prisma.chatRoom.findMany({
        where: {
          type: "GROUP",
          id: memberRoomIds.length ? { notIn: memberRoomIds } : undefined,
        },
        orderBy: [{ lastMessageAt: "desc" }, { updatedAt: "desc" }],
        include: roomInclude,
      });

      return res.status(200).json({ rooms: [...memberRooms, ...adminRooms] });
    }
      ...
      if (type === "DIRECT") {
        ...
        const room = await prisma.chatRoom.create({
          data: {
            ...
            members: {
              create: [{ userId: req.session.userId }, { userId: otherUserId }],
            },
          },
        });
      ...
      if (type === "GROUP") {
        ...
        const room = await prisma.chatRoom.create({
          data: {
            ...
            createdById: req.session.userId,
            members: {
              create: [{ userId: req.session.userId }, ...cleanedMemberIds.map((userId) => ({ userId }))],
            },
          },
        });
